...
Mooltiproxy is an abstraction layer allowing to expose one API endpoints while adressing another API in the background. It can thus be seen as an API translation proxy.

It is a pico-framework (can't even name it micro or nano) with very basic features, and being a personal pet project, it obiously cannot compete with more complete solutions, but it is self-contained with minimal dependencies (requests for serving requests, pyyaml and voluputous for config file validation), and as such allows to **rapidly experiment switching from one api to another without changing your client code**.
